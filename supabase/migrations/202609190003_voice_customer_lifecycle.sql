-- AnaAI voice customer lifecycle integration.
--
-- Voice booking remains a trusted service_role-only boundary.
-- This migration preserves the existing voice booking contract while making
-- customer lifecycle behavior consistent with the CRM archive model.
--
-- A returning caller whose phone matches an archived customer is reactivated
-- and reused rather than creating a duplicate customer. This preserves the
-- customer's historical appointment linkage under the same customer id.
