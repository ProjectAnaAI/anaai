...
function harness({initial='Booked',sms='accepted',deny=false}={}){
 ...
 const route=load('app/api/appointments/route.ts',{'@/lib/appointment-actions':helpers,'@/lib/business-context':{resolveBusinessContext:async()=>deny?{success:false,status:403,error:'Denied'}:{success:true,context:{businessId:business}}},'@supabase/supabase-js':{createClient:()=>db},'next/server':{NextResponse:{json:(v,o)=>Response.json(v,o)}}});
