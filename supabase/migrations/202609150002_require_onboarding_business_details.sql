...
-- Tighten onboarding validation so every newly provisioned business must
-- provide a business phone number, email address, and physical address.
...
-- Preserve the provisioning RPC's restricted execution permissions.
revoke all
on function public.create_business_for_current_user(jsonb)
from public, anon;
