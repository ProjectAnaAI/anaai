...
export async function saveCustomer(
  ...
  try {
    ...
    if (phoneKey) {
      /*
       * Supabase caps result sets. Check every page rather than
       * silently missing an existing customer beyond the first page.
       *
       * Archived customers remain part of the CRM identity set, so
       * they participate in duplicate-phone detection too.
       */
      const pageSize = 500;
